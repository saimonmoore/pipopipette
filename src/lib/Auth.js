...
class Auth {
  ...
  login(user_id, fn) {
    ...
    this.auth().signInWithEmailAndPassword(this.email(user_id), this.token).catch(function(error) {
  ...
  signup(user_id, fn) {
    try {
      this.auth().createUserWithEmailAndPassword(this.email(user_id), this.token).then((foo)=> {
        fn(null, foo)
