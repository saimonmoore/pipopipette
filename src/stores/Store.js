...
class Store {
  ...
  handleLineAdded(data) {
    ...
    if (!Line.exists(this.lines, line)) {
      console.log(
        "[Store#handleLineAdded]...line doesn't already exist...adding line "
      );
      this.addLine(line, line.player);
      console.log(
        "[Store#handleLineAdded]...line doesn't already exist...added line "
      );
    }
  }
  ...
  handleBoxChanged(data) {
    ...
    console.log(
      '[Store#handleBoxChanged]...box: ',
      box && JSON.stringify(toJS(box))
    );
  ...
  handleGridSizeChanged(grid_size) {
    ...
    if (this.grid_size.get() !== grid_size) {
      console.log(
        '[Store#handleGridSizeChanged]...changing grid_size: ',
        this.grid_size.get(),
        grid_size
      );
      this.changeGridSize(grid_size, false);
      console.log(
        '[Store#handleGridSizeChanged]...changed grid_size...now fetching data...'
      );
      ...
      this.fetchData()
        .then(data => {
          ...
          console.log(
            '[Store#handleGridSizeChanged]...in fetchData#fn... mustLoadData: ',
            mustLoadData
          );
  ...
  handlePlayerChanged(data) {
    ...
    if (Player.isPlayerTwo(player, this.player1)) {
      ...
      console.log(
        '[Store#handlePlayerChanged]...was player 2: ',
        JSON.stringify(toJS(this.player2))
      );
    }
  }
  ...
  handlePlayerAdded(data) {
    ...
    if (Player.isPlayerTwo(player, this.player1)) {
      ...
      console.log(
        `[Store#handlePlayerAdded]...was player 2...game now running...turn: ${this.turn.get()}`
      );
    }
  }
  ...
  firstTimeActions() {
    ...
    if (!colour || !colour.length) {
      console.log(
        '[Store#firstTimeActions]...assigning random colour to player1'
      );
  ...
  saveSession(session) {
    ...
    console.log(
      '[Store#saveSession]...about to call `fetchData` session: ',
    ...
    this.fetchData()
      .then(data => {
        ...
        const mustLoadData = lines.length || boxes.length;
        console.log(
          '[Store#saveSession]...in `fetchData#fn` mustLoadData: ',
          mustLoadData
        );
        ...
        if (session) {
          Object.assign(this.session, session);
          console.log(
            '[Store#saveSession]...in `fetchData#fn` got session: ',
            JSON.stringify(session)
          );
          ...
          if (
            ...
            console.log(
              '[Store#saveSession]...in `fetchData#fn` changing grid_size to: ',
              session.grid_size
            );
          ...
          if (session.turn) {
            console.log(
              '[Store#saveSession]...in `fetchData#fn` got turn from session...setting to: ',
              session.turn
            );
            this.turn.set(session.turn);
          } else {
            console.log(
              "[Store#saveSession]...in `fetchData#fn` didn't get turn from session...setting to player 1's turn: ",
              this.player1.id
            );
          ...
          if (session.status) {
            console.log(
              '[Store#saveSession]...in `fetchData#fn` got status from session...setting to: ',
              session.status
            );
            this.status.set(session.status);
          } else {
            console.log(
              "[Store#saveSession]...in `fetchData#fn` didn't get status from session...setting to `waiting `"
            );
        ...
        // Load players
        console.log(
          '[Store#saveSession]...in `fetchData#fn` loading players...'
        );
        players &&
          Object.values(players).forEach(data => {
            ...
            if (Player.isPlayerOne(player, this.player1)) {
              ...
              console.log(
                '[Store#saveSession]...in `fetchData#fn` loading player 1: ',
                JSON.stringify(data),
                JSON.stringify(toJS(this.player1))
              );
            }
            ...
            if (Player.isPlayerTwo(player, this.player1)) {
              ...
              this.player2 = player;
              console.log(
                '[Store#saveSession]...in `fetchData#fn` loading player 2: ',
                JSON.stringify(data),
                JSON.stringify(toJS(this.player2))
              );
            }
      ...
      .catch(error => {
        ...
        console.log(
          '[Store#saveSession]...error `fetchData` error: ',
          JSON.stringify(error)
        );
        this.firstTimeActions();
      });
